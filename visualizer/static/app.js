...
function render() {
  ...
  const labels = labelLayer.selectAll("text")
    .data(graph.links.filter(link => link.kind === "relates_to"), d => d.id)
    .join("text")
  ...
  nodes.select("text").text(d => d.name || d.id);
...
function renderNodeDetails(node) {
  ...
  detailsEl.innerHTML = `
    ...
    <section class="detail-block">
      <h3>Relationships</h3>
      ${incident.length ? incident.slice(0, 18).map(edgeSummary).join("") : `<p class="empty">No visible relationships.</p>`}
    </section>
    <section class="detail-block">
      <h3>Neighbors</h3>
